...
# 1) Parse the user’s search_prompt into query_specs
...
try:
    ...
    st.error("❗ Please enter your query as JSON, e.g.\n"
             "`{\"isopropanol\":{\"price\":10,\"match_score\":1.0},"
             "\"acetone\":{\"price\":20,\"match_score\":0.8}}`")
    st.stop()

# 2) Make sure we have results
if 'search_results' not in st.session_state:
    st.info("Perform a search to see the vendor ranking.")
    st.stop()

# 3) Build supply_specs from the search results
...
all_e=list(G.edges())
opt_e=[e for e in all_e if e[1] in best_subset]
...
nx.draw_networkx_nodes(G,pos,nodelist=query_specs.keys(), node_shape='o',
                       node_color='#4C72B0',node_size=800,ax=ax)
base_v=[v for v in vendor_scores['vendor'] if v not in best_subset]
nx.draw_networkx_nodes(G,pos,nodelist=base_v, node_shape='s',
                       node_color='#55A868',node_size=600,ax=ax)
nx.draw_networkx_nodes(G,pos,nodelist=best_subset, node_shape='s',
                       node_color='#C44E52',node_size=1000,edgecolors='black',linewidths=2,ax=ax)
